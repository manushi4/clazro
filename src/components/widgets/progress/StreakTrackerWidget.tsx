/**
 * Streak Tracker Widget (streak.tracker)
...
const WIDGET_ID = "streak.tracker";

export const StreakTrackerWidget: React.FC<WidgetProps> = ({ config, onNavigate, size = "standard" }) => {
    ...
    return (
      <View style={[styles.container, styles.centerContainer]}>
        ...
        <AppText style={[styles.stateText, { color: colors.onSurfaceVariant }]}>
          {t("widgets.studyStreak.states.loading")}
        </AppText>
      </View>
    );
  }
    ...
    return (
      <View style={[styles.container, styles.centerContainer]}>
        ...
        <AppText style={[styles.stateText, { color: colors.onSurface }]}>
          {t("widgets.studyStreak.states.error")}
        </AppText>
        <TouchableOpacity
        ...
        >
          <AppText style={[styles.retryText, { color: colors.onPrimary }]}>
            {t("widgets.studyStreak.actions.retry")}
          </AppText>
        </TouchableOpacity>
      </View>
    );
  ...
  if (!data) {
    return (
      <View style={[styles.container, styles.centerContainer]}>
        ...
        <AppText style={[styles.stateText, { color: colors.onSurfaceVariant }]}>
          {t("widgets.studyStreak.states.empty")}
        </AppText>
      </View>
    );
  }
  ...
  return (
    <View style={styles.container}>
      ...
      {showCurrentStreak && (
        <TouchableOpacity
          style={[styles.streakSection, { backgroundColor: colors.surfaceVariant, borderRadius: borderRadius.medium }]}
          onPress={enableTap ? handleStreakPress : undefined}
          disabled={!enableTap}
          activeOpacity={0.7}
        >
          <View style={styles.streakHeader}>
            <View style={styles.streakInfo}>
              ...
              <View style={styles.streakLabels}>
                <AppText style={[styles.streakTitle, { color: colors.onSurface }]}>
                  {t("widgets.studyStreak.labels.currentStreak")}
                </AppText>
                <AppText style={[styles.streakSubtitle, { color: colors.onSurfaceVariant }]}>
                  {t("widgets.studyStreak.labels.days", { count: data.current_streak })}
                </AppText>
              </View>
            </View>
            ...
            {showWeeklyGoal && (
              <View style={styles.goalProgress}>
                <View style={[styles.progressBar, { backgroundColor: `${colors.outline}30`, borderRadius: borderRadius.small }]}>
                  <View
                    style={[
                      styles.progressBarFill,
                      { backgroundColor: streakColor, width: `${weeklyProgress}%`, borderRadius: borderRadius.small },
                    ]}
                  />
                </View>
                <AppText style={[styles.goalText, { color: colors.onSurfaceVariant }]}>
                  {data.current_streak}/{data.weekly_goal} {t("widgets.studyStreak.labels.weeklyGoal")}
                </AppText>
              </View>
            )}
          </View>
          ...
          {showLongestStreak && (
            <View style={[styles.statCard, { backgroundColor: colors.surfaceVariant, borderRadius: borderRadius.medium }]}>
              ...
              <AppText style={[styles.statLabel, { color: colors.onSurfaceVariant }]}>
                {t("widgets.studyStreak.labels.longest")}
              </AppText>
            </View>
          )}
          {size === "expanded" && (
            <>
              <View style={[styles.statCard, { backgroundColor: colors.surfaceVariant, borderRadius: borderRadius.medium }]}>
                ...
                <AppText style={[styles.statLabel, { color: colors.onSurfaceVariant }]}>
                  {t("widgets.studyStreak.labels.totalDays")}
                </AppText>
              </View>
              <View style={[styles.statCard, { backgroundColor: colors.surfaceVariant, borderRadius: borderRadius.medium }]}>
                <Icon name="clock" size={16} color={colors.info} />
                <AppText style={[styles.statValue, { color: colors.onSurface }]}>
                  {formatHours(data.total_study_hours)}
                </AppText>
                <AppText style={[styles.statLabel, { color: colors.onSurfaceVariant }]}>
                  {t("widgets.studyStreak.labels.totalHours")}
                </AppText>
              </View>
            </>
          )}
      ...
      {showAchievements && data.achievements.length > 0 && (
        <View style={styles.achievementsSection}>
          <AppText style={[styles.sectionTitle, { color: colors.onSurface }]}>
            {t("widgets.studyStreak.labels.recentAchievements")}
          </AppText>
          <ScrollView
            horizontal
          ...
          >
            {data.achievements.slice(0, maxAchievements).map((achievement) => (
              <TouchableOpacity
                key={achievement.id}
                style={[styles.achievementCard, { backgroundColor: colors.surfaceVariant, borderRadius: borderRadius.medium }]}
                onPress={enableTap ? () => handleAchievementPress(achievement) : undefined}
                disabled={!enableTap}
                activeOpacity={0.7}
              >
                <View
                  style={[
                    ...
                    { backgroundColor: `${getAchievementColor(achievement.color)}20`, borderRadius: 20 },
                  ]}
                >
                  <Icon name={achievement.icon} size={20} color={getAchievementColor(achievement.color)} />
                </View>
                <AppText style={[styles.achievementTitle, { color: colors.onSurface }]} numberOfLines={1}>
                  {getLocalizedField(achievement, "title")}
                </AppText>
                <AppText style={[styles.achievementDesc, { color: colors.onSurfaceVariant }]} numberOfLines={2}>
                  {getLocalizedField(achievement, "description")}
                </AppText>
              </TouchableOpacity>
            ))}
          </ScrollView>
      ...
      {/* Recent Activity Section (expanded only) */}
      {showRecentActivity && data.recent_activities.length > 0 && (
        <View style={styles.activitySection}>
          <AppText style={[styles.sectionTitle, { color: colors.onSurface }]}>
            {t("widgets.studyStreak.labels.recentActivity")}
          </AppText>
          {data.recent_activities.slice(0, 3).map((activity) => (
            <View
              key={activity.id}
              style={[styles.activityItem, { backgroundColor: colors.surfaceVariant, borderRadius: borderRadius.medium }]}
            >
              <View style={styles.activityHeader}>
                <AppText style={[styles.activityDate, { color: colors.onSurface }]}>
                  {new Date(activity.study_date).toLocaleDateString()}
                </AppText>
                <AppText style={[styles.activityDuration, { color: colors.primary }]}>
                  {formatHours(activity.minutes_studied / 60)}
                </AppText>
              </View>
              <AppText style={[styles.activityType, { color: colors.onSurfaceVariant }]}>
                {getLocalizedField(activity, "activity_type")}
              </AppText>
              <AppText style={[styles.activitySubjects, { color: colors.onSurfaceVariant }]}>
                {activity.subjects_studied.join(", ")} • {activity.activities_completed}{" "}
                {t("widgets.studyStreak.labels.activities")}
              </AppText>
            </View>
          ))}
        </View>
      ...
      {showMotivation && data.milestone_title_en && (
        <View style={[styles.motivationCard, { backgroundColor: `${streakColor}15`, borderRadius: borderRadius.medium }]}>
          <Icon name="star" size={16} color={streakColor} />
          <View style={styles.motivationText}>
            <AppText style={[styles.motivationTitle, { color: colors.onSurface }]}>
              {getLocalizedField(data, "milestone_title")}
            </AppText>
            <AppText style={[styles.motivationDesc, { color: colors.onSurfaceVariant }]}>
              {getLocalizedField(data, "milestone_description")}
            </AppText>
          </View>
        </View>
      )}
      ...
      {enableTap && (
        ...
        >
          <AppText style={[styles.viewAllText, { color: colors.primary }]}>
            {t("widgets.studyStreak.actions.viewAll")}
          </AppText>
...
const styles = StyleSheet.create({
  ...
  achievementCard: { width: 120, padding: 12, alignItems: "center", gap: 6 },
  achievementIcon: { width: 40, height: 40, alignItems: "center", justifyContent: "center" },
  ...
  motivationCard: { flexDirection: "row", alignItems: "center", padding: 12, gap: 10 },
